...
# print([hex(val) for val in testing]) #for converting a list to their hex values
...
# def list_to_array(msglist):
#     array = np.array([[msglist[0], msglist[4], msglist[8], msglist[12]],[msglist[1], msglist[5], msglist[9], msglist[13]],[msglist[2], msglist[6], msglist[10], msglist[14]],[msglist[3], msglist[7], msglist[11], msglist[15]]])
#     return array

# def text_to_array(text):
#     message_list = text_to_list(text)
#     array = list_to_array(message_list)
#     return array

def split_to_16_bytes(plaintext):
    message_list = []

    #our message does NOT split up evenly (base case)
    ...
    #our message DOES split evenly (edge case)
    if len(plaintext) % 16 == 0:
        remaining_length = len(plaintext)
        i, j = 0, 16
...
def text_to_list(m_list):
    new_message_list = []
    i, j = 0, 2
    for _ in range(len(m_list) // 2): # // operator discards remainder
...
### Below is an OPTIMIZED version of split_message yet to be used ###
# def split_message(plaintext):
#     message_list = []
...
plaintext = "base message txt but what if it doesn't quite work"
message_list = split_to_16_bytes(plaintext)
print(message_list)
#NEED TO EDIT text_to_list function so it works with our new list of text that looks like: ['62617365206d65737361676520747874', '20627574207768617420696620697420']

# corpus = plaintext.encode('utf-8').hex()
# corpus = text_to_list(corpus)
# print(corpus)
# sub_bytes(corpus)
# shift_rows(corpus)
# mix_columns(corpus)
